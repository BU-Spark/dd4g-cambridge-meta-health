...
- **Quality Evaluation** — Comprehensive assessment of metadata completeness, freshness, and consistency
- **Human-in-the-Loop Approval Workflow** — Interactive interface for reviewing and managing dataset metadata with full audit trail
...
**Root Level:**
* **`pipeline.py`** — Legacy/alternative pipeline orchestrator
  - Entry point for running the full data processing workflow
  - Can be used as alternative to `ETL/pipeline.py`

**Data Ingestion:**
* **`EDA/fetch_data.py`** — EDA data ingestion layer
  - Fetches dataset metadata from Cambridge Open Data Portal
  - Implements Socrata API client with paginated requests
  - Normalizes and persists raw metadata to SQLite

* **`ETL/ingest.py`** — ETL data ingestion pipeline
  - Fetches dataset metadata from Cambridge Open Data Portal (Socrata Discovery API)
  - Handles paginated API fetching with intelligent upsert
  - Stores metadata in SQLite with JSON fields for arrays
  - Manages database initialization and indexes

**ETL & Processing:**
* **`ETL/pipeline.py`** — ETL pipeline orchestrator
  - Coordinates data ingestion, evaluation, and publishing
  - Manages complete data processing workflow
  - Handles error recovery and state management

* **`ETL/evaluate.py`** — Quality evaluation engine
  - Applies multi-dimensional quality scoring algorithm
  - Computes health scores and band classifications (Critical/Poor/Fair/Good)
  - Generates quality metrics and flags for datasets
  - Evaluates metadata completeness and freshness

* **`ETL/publish.py`** — Publishing and deployment automation
  - Handles database finalization and output generation
  - Prepares data for dashboard consumption
  - Manages data export and reporting
...
**Utilities & Setup:**
* **`ETL/setup/`** — Database and environment setup utilities
  - `config.py` — Configuration management
  - `reset_db.py` — Database initialization and reset
  - `to_excel.py` — Data export to Excel format

* **`ETL/evaluate_scripts/`** — Legacy evaluation scripts (archived)
  - Contains previous versions of scoring and enrichment algorithms
...
```bash
# Clone repository
git clone https://github.com/BU-Spark/dd4g-cambridge-meta-health
cd dd4g-cambridge-meta-health
...
1. **Initialize System** — Run `python ETL/pipeline.py` (10-30 min depending on dataset size)
...
The system implements a **multi-stage ETL (Extract-Transform-Load) pipeline** with clear separation of concerns and a human-in-the-loop approval stage:
...
Data Source Layer
    Cambridge Open Data Portal (Socrata API)
              ↓
Ingestion Layer
    [Stage 1] EDA/fetch_data.py — Data extraction & normalization
              ↓
         datasets table (raw ingested data)
              ↓
Processing Layer
    [Stage 2] ETL/evaluate.py — Quality assessment & transformation
              ↓
         health_flags table (processed metrics)
              ↓
Presentation & Approval Layer
    [Stage 3] Streamlit UI — Interactive review workflow
              ↓
...
**Pipeline Orchestration:** The `ETL/pipeline.py` script manages the orchestration of all stages, handling dependencies, error recovery, and state management across the entire workflow.
...
```bash
# Syntax check
python -m py_compile EDA/*.py ETL/*.py streamlit/app.py
...
**Manual Refresh:** `python ETL/pipeline.py && python ETL/publish.py`
